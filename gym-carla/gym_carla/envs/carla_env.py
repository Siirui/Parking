...
class CarlaEnv(gym.Env):
  ...
  def __init__(self, params):
    ...
    #   self.dests = [[-29.2, -24.2]]
      self.dests = [[290.9, -210.8]]
    else:
      self.dests = None

    self.dest_orientation = 0
    self.initial_position = [285.4, -237.8]
    self.last_step_position = self.initial_position
  ...
  def reset(self): 
   ...
   # Clear sensor objects
    # print(self.collision_sensor, self.lidar_sensor, self.camera_sensor)
    # print(self.collision_sensor.is_listening, self.lidar_sensor.is_listening, self.camera_sensor.is_listening)
   ...
   # Delete sensors, vehicles and walkers 
    ...
    # self.waxlker_polygons.append(walker_poly_dict)
    ...
    while True:
      ...
      if self.task_mode == 'roundabout':
        ...
        # self.start=[-48.7, -30.9, 0] # static TOWN05
        self.start=[285.4, -237.8, 90] # static
    ...
    # print("finish spawn ego vehicle")
  ...
  def _get_obs(self):
    ...
    state = np.array([ego_x, ego_y, self.dests[0][0], self.dests[0][1], self.dests[0][0] - ego_x, self.dests[0][1] - ego_y])
  ...
  def _get_reward(self):
    ...
    # dot_product = v.x * forward_vector.x + v.y * forward_vector.y + v.z * forward_vector.z
    # forward_speed = math.sqrt(dot_product ** 2)
    ...
    pre_dx = (self.last_step_position[0] - dest[0]) ** 2
    pre_dy = (self.last_step_position[1] - dest[1]) ** 2
    pre_dis = np.sqrt(pre_dx + pre_dy)

    self.last_step_position = [ego_x, ego_y]
    r_dis = pre_dis - dis

    # print("speed:", forward_speed)
    
    timeout = self.time_step>self.max_time_episode
  ...
  def _terminal(self):
    ...
    if self.dests is not None: # If at destination
      for dest in self.dests:
        if np.sqrt((ego_x-dest[0])**2+(ego_y-dest[1])**2)<4 and np.abs(ego_yaw - self.dest_orientation) < 5:
          print("destination")
          return True
